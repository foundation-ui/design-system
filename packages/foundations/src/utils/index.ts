import { Hex, Luminance, Tone, Mode } from '../../../types';

/** Format convertors */
export const pxTorem = (value: number) => Number(0.0625 * value);
export const pxToPt = (value: number) => Number(0.75 * value);
export const checkDecimals = (value: number) => value === Math.floor(value);
export const formatDecimals = (value: number) => Number(value.toFixed(1));
...
export const convertToHSL = (hex: string, raw?: boolean) => {
	const cleanHex = hex.replace('#', '');
	const parsedHEXtoRGB = convertToRGB(cleanHex, true);
...
export const calculateContrastScore = (
	...
	const textRgb = convertToRGB(textColor, true);
	const backgroundRgb = convertToRGB(backgroundColor, true);
